...
生产化三块（详见 [M5 验收报告](./M5-验收报告.md)）：

| 组件 | 职责 |
|---|---|
| `CapabilityRegistry` | 能力目录持久化 + **双方审批变更**：单方审批不生效，双方批准后 ACTIVE 并对业务可见、旧版本退休 |
| `MetricsCollector` + `/pps/v1/metrics` | 按能力/状态计数 + 延迟，Prometheus 文本导出（NOOP 默认零开销，重载构造注入） |
| 多应用隔离 | 应用级配额独立、跨应用同 reqId 不构成重放、跨应用作业不可见（幂等键作用域 = app_id+req_id） |
| SDK v1 `awaitResult` | 批处理便捷（提交+轮询+取结果一步到位），在线路径仍推荐回调 |

**门禁**：能力治理 2 项（单批不生效/双批激活替换）+ 可观测与隔离 3 项。全量 96 项测试全绿。
交付《M5 业务系统接入手册》《M5 验收报告》。
